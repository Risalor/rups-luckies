...
export default class WorkspaceSceneLogicGates extends Phaser.Scene {
  ...
  create() {
    ...
    const title = this.add
      .text(panelWidth / 2, 60, 'LOGIC GATES', {
        fontSize: '24px',
        color: '#00ffcc',
        fontStyle: 'bold',
    ...
    this.timerText = this.add
      .text(width - 120, 30, `⏱ ${this.formatTime(this.timerCountdown)}`, {
        fontSize: '28px',
        color: '#00ffcc',
        fontStyle: 'bold',
        fontFamily: 'Arial, sans-serif',
    ...
    this.challenges = [
      ...
      },
      {
        ...
        solutionCheck: (circuit) => {
          ...
          const criticalTests = [testCases[0], testCases[1], testCases[2], testCases[3]];
      ...
      },
      {
        ...
        solutionCheck: (circuit) => {
          ...
          for (const [id, gate] of circuit.gates) {
            if (gate.operation === 'XOR' || gate.operation === 'AND' || gate.operation === 'OR' || gate.operation === 'NOT') {
              return false;
            }
          }
      ...
      },
      {
        ...
        solutionCheck: (circuit) => {
          ...
          const criticalTests = [testCases[0], testCases[1], testCases[2], testCases[3], testCases[4], testCases[6], testCases[7]];
    ...
    this.challengeSolutions = [
      ...
      () => {
        ...
        const inputs = [this.createGateOnGrid(startX, startY - 120, 'input', 'INPUT A'), this.createGateOnGrid(startX, startY - 40, 'input', 'INPUT B'), this.createGateOnGrid(startX, startY + 40, 'input', 'INPUT C'), this.createGateOnGrid(startX, startY + 120, 'input', 'INPUT D')];
    ...
    this.taskText = this.add
      .text(taskBoxX + 15, taskBoxY + 15, this.challenges[this.currentChallengeIndex].prompt, {
        fontSize: '16px',
        color: '#ffffff',
        fontFamily: 'Arial, sans-serif',
        fontStyle: 'bold',
  ...
  updateTimer() {
    ...
    if (this.timerCountdown <= 0) {
      ...
      const flash = this.add.rectangle(0, 0, this.cameras.main.width, this.cameras.main.height, 0xff0000, 0.2).setOrigin(0).setDepth(999);
  ...
  updateBulbAppearance(container, isOn) {
    ...
    if (isOn) {
      if (!this.bulbGlows.has(gateId)) {
        const glow = this.add.circle(0, 0, 50, 0xffff00, 0.3).setDepth(-1);
  ...
  showPinTooltip(pin, container, isOutput) {
    ...
    try {
      ...
      const tooltipText = this.add
        .text(pinWorldX, pinWorldY - 20, pinValue ? 'TRUE' : 'FALSE', {
          fontSize: '16px',
  ...
  drawConnectionWithBend(fromX, fromY, toX, toY, fromId, toId, toPinIndex) {
    ...
    const hit = this.add
      .zone(midPointX, midPointY, Math.max(40, Math.abs(toX - fromX) / 2), 20)
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true });
  ...
  setupInputOutputPins(container, type, labelText, img) {
    ...
    if (!inputPins || inputPins.length === 0) {
      ...
      for (let i = 0; i < maxInputs; i++) {
        ...
        const inPin = this.add.circle(-36, yOff, pinRadius, inputPinColor).setStrokeStyle(2, 0xffffff, 0.8).setOrigin(0.5);
    ...
    if (type !== 'bulb' && !outputPin) {
      outputPin = this.add.circle(36, 0, pinRadius, outputPinColor).setStrokeStyle(2, 0xffffff, 0.8).setOrigin(0.5);
  ...
  createComponent(x, y, type, labelText, color) {
    ...
    const img = this.add.image(0, 0, textureName).setDisplaySize(size, size).setOrigin(0.5).setAlpha(1);
    container.add(img);
    container.setData('img', img);

    const label = this.add
      .text(0, 40, labelText || type, {
        fontSize: '14px',
        color: '#ffffff',
    ...
    try {
      this.connections = this.connections.filter((conn) => {
        if (conn.fromId === gateId || conn.toId === gateId) {
          try {
            if (conn.gfx) conn.gfx.destroy();
          } catch (e) {}
          try {
            if (conn.glow) conn.glow.destroy();
          } catch (e) {}
          try {
            if (conn.hitZone) conn.hitZone.destroy();
          } catch (e) {}
          return false;
        }
        return true;
